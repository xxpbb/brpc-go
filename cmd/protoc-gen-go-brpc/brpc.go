...
func (serviceGenerateHelper) generateClientStruct(g *protogen.GeneratedFile, clientName string) {
	...
	g.P("cc ", brpcPackage.Ident("ClientConnInterface"))
	g.P("}")
	g.P()
}
...
// generateFile generates a _brpc.pb.go file containing gRPC service definitions.
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	g.P("// Code generated by protoc-gen-go-brpc. DO NOT EDIT.")
	g.P("// versions:")
	g.P("// - protoc-gen-go-brpc v", version)
...
// generateFileContent generates the bRPC service definitions, excluding the package statement.
func generateFileContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile) {
	...
	g.P("// is compatible with the brpc package it is being compiled against.")
	g.P("// Requires bRPC-Go v0.1.0 or later.")
	g.P("const _ = ", brpcPackage.Ident("SupportPackageIsVersion1")) // When changing, update version number above.
	g.P()
...
func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	g.P("func New", clientName, " (cc ", brpcPackage.Ident("ClientConnInterface"), ") ", clientName, " {")
	...
	g.P("func Register", service.GoName, "Server(s ", brpcPackage.Ident("ServiceRegistrar"), ", srv ", serverType, ") {")
...
func clientSignature(g *protogen.GeneratedFile, method *protogen.Method) string {
	...
	s += ", opts ..." + g.QualifiedGoIdent(brpcPackage.Ident("CallOption")) + ") ("
	if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
...
func genClientMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, index int) {
	...
	g.P(brpcPackage.Ident("ClientStream"))
	g.P("}")
	g.P()
...
func genServiceDesc(file *protogen.File, g *protogen.GeneratedFile, serviceDescVar string, serverType string, service *protogen.Service, handlerNames []string) {
	// Service descriptor.
	g.P("// ", serviceDescVar, " is the ", brpcPackage.Ident("ServiceDesc"), " for ", service.GoName, " service.")
	g.P("// It's only intended for direct use with ", brpcPackage.Ident("RegisterService"), ",")
	g.P("// and not to be introspected or modified (even as a copy)")
	g.P("var ", serviceDescVar, " = ", brpcPackage.Ident("ServiceDesc"), " {")
	g.P("ServiceName: ", strconv.Quote(string(service.Desc.FullName())), ",")
	g.P("HandlerType: (*", serverType, ")(nil),")
	g.P("Methods: []", brpcPackage.Ident("MethodDesc"), "{")
...
func genServerMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, hnameFuncNameFormatter func(string) string) string {
	...
	g.P("func ", hnameFuncNameFormatter(hname), "(srv interface{}, stream ", brpcPackage.Ident("ServerStream"), ") error {")
	if !method.Desc.IsStreamingClient() {
	...
	g.P(brpcPackage.Ident("ServerStream"))
	g.P("}")
	g.P()
